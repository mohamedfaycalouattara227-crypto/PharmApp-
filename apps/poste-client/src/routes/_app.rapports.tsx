/**
 * _app.rapports.tsx — Étape 8 : Rapports & Statistiques
 *
 * Onglets :
 *  1. Ventes — CA par période, répartition par mode de paiement, top produits
 *  2. Top produits — classement volume sur N jours
 *  3. Marges — marge brute par produit (Titulaire uniquement)
 *  4. Export — téléchargement CSV ventes
 *
 * Graphiques : barres SVG inline (pas de dépendance externe recharts).
 */
...
function getDateDebut(joursMoins: number): string {
  const d = new Date();
  d.setDate(d.getDate() - joursMoins);
...
function OngletVentes() {
  ...
  const repartition = data?.repartition_paiement ?? {};
  const chartData = Object.entries(repartition)
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end gap-3">
        <div>
          ...
          <input className="input w-40" type="date" value={debut} onChange={(e) => setDebut(e.target.value)} />
        </div>
        <div>
          <label className="label">Au</label>
          <input className="input w-40" type="date" value={fin} onChange={(e) => setFin(e.target.value)} />
        </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          {/* Top produits */}
          {(data.top_produits?.length ?? 0) > 0 && (
            <div className="rounded-xl border border-border/70 bg-surface/40 p-5">
              <h3 className="font-medium mb-4">Top 10 produits sur la période</h3>
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="border-b border-border/70 text-muted-foreground text-left">
                    ...
                    <th className="pb-2 pr-4 font-medium text-right">Quantité vendue</th>
                ...
                <tbody className="divide-y divide-border/40">
                  {(data.top_produits ?? []).map((p, i) => (
                    <tr key={i} className="hover:bg-surface/60">
                      ...
                      <td className="py-2 pr-4 font-medium">{p.medicament__nom}</td>
                      <td className="py-2 pr-4 text-right font-mono">{fmtNombre(p.quantite_totale)}</td>
                      <td className="py-2 text-right font-mono">{fmtFCFA(p.ca ?? p.chiffre_affaires ?? 0)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-muted-foreground">Aucune donnée sur cette période.</div>
      ) : (
...
// ── Onglet Marges (Titulaire uniquement) ─────────────────────────────────────
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-muted-foreground">Aucune donnée de marge sur cette période.</div>
      ) : (
        <div className="rounded-xl border border-border/70 overflow-hidden">
          <table className="w-full text-sm border-collapse">
            <thead className="bg-surface/80">
            ...
            <tbody className="divide-y divide-border/40">
              {(data as MargeProduit[]).map((m, i) => (
                <tr key={i} className="hover:bg-surface/60">
                  ...
                  <td className={`py-2.5 pr-4 text-right font-mono ${Number(m.marge_brute ?? m.marge) < 0 ? "text-destructive" : "text-success"}`}>
                    {fmtFCFA(m.marge_brute ?? m.marge ?? 0)}
                  </td>
                  <td className={`py-2.5 pr-4 text-right ${Number(m.taux_marge_pct) < 10 ? "text-warning" : "text-foreground"}`}>
                    {m.taux_marge_pct} %
                  </td>
                </tr>
              ))}
  ...
  return (
    <div className="space-y-6 max-w-lg">
      <p className="text-sm text-muted-foreground">
        Téléchargez le détail de toutes les ventes validées au format CSV (compatible Excel / LibreOffice Calc).
      </p>
      ...
      <a
        href={api.rapports.exportVentesUrl(debut, fin)}
        download={`ventes_${debut}_${fin}.csv`}
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-auto px-6 py-6">
        ...
        {onglet === "marges" && (peutVoirMarges() ? <OngletMarges /> : (
          <div className="text-center py-16 text-muted-foreground">
            Accès réservé au pharmacien titulaire.
          </div>
        ))}
