/**
 * Historique des ventes + création d'avoir (retour produit).
 * Route : /_app/ventes
 *
 * Étape 14 — CDC §4.4.3
 *
 * Fonctionnalités :
 *   - Liste paginée des ventes avec filtres (date, statut, mode paiement)
 *   - Badge statut (validée, annulée, avoir)
 *   - Dialog "Créer un avoir" : sélection des lignes à retourner, motif, confirmation
 */
...
export const Route = createFileRoute("/_app/ventes")({
  ...
  head: () => ({
    meta: [{ title: "Historique des ventes — PharmApp" }],
  }),
});
...
function DialogAvoir({
  ...
  const mutation = useMutation({
    mutationFn: () =>
      api.ventes.avoir(vente.id, { lignes: lignesRetour, motif }),
    onSuccess: (data) => {
      toast.success("Avoir créé", {
        description: `${data.total_avoir} FCFA remboursés.`,
      });
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="font-display text-xl">
            Créer un avoir — Vente {vente.numero}
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-5 py-2">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {lignes.map((l) => {
                  ...
                  const montantLigne =
                    qte * Number(l.prix_unitaire_apres_remise ?? l.prix_unitaire ?? 0);

                  return (
                    <tr key={ligneId} className="hover:bg-muted/10">
                      <td className="px-4 py-3">
                        <span className="font-medium">{l.medicament_nom ?? l.medicament}</span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          <button
                            type="button"
                            onClick={() => setQte(ligneId, Math.max(0, qte - 1))}
                            className="h-7 w-7 flex items-center justify-center rounded border hover:bg-surface-strong"
                          >
                          ...
                          <Input
                            ...
                            value={qte}
                            onChange={(e) =>
                              setQte(ligneId, Math.min(maxQte, Math.max(0, Number(e.target.value))))
                            }
                            className="w-14 text-center h-7 text-sm"
                          />
                          <button
                            type="button"
                            onClick={() => setQte(ligneId, Math.min(maxQte, qte + 1))}
                            className="h-7 w-7 flex items-center justify-center rounded border hover:bg-surface-strong"
                          >
                        ...
                        {qte > 0 && (
                          <p className="text-right text-xs text-muted-foreground mt-0.5 tabular-nums">
                            {fmtFCFA(montantLigne)}
                          </p>
                        )}
                      </td>
                    </tr>
          ...
          {totalAvoir > 0 && (
            <div className="flex items-baseline justify-between rounded-lg bg-amber-50 border border-amber-200 px-4 py-3">
              ...
              <span className="numeric font-display text-xl text-amber-900">
                {fmtFCFA(totalAvoir)}
              </span>
            </div>
          )}
          ...
          <div>
            ...
            <Input
              className="mt-2"
              placeholder="Ex : Produit endommagé, erreur de délivrance, périmé…"
              value={motif}
        ...
        <DialogFooter>
          <Button variant="outline" onClick={onFermer} disabled={mutation.isPending}>
            Annuler
          </Button>
          <Button
    ...
    <div className="flex flex-col gap-6 p-6 max-w-screen-xl mx-auto">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Retrouvez toutes les ventes, annulations et avoirs.
        </p>
      </div>
                  ...
                  <>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-muted-foreground capitalize">
                        {v.mode_paiement?.replace(/_/g, " ") ?? "—"}
                      </td>
                      <td className="px-4 py-3 text-right tabular-nums font-medium">
                        {fmtFCFA(Number(v.montant_total))}
                      </td>
      ...
      {venteAvoir && (
        <DialogAvoir
          vente={venteAvoir}
          onFermer={() => setVenteAvoir(null)}
          onSuccess={() => setVenteAvoir(null)}
        />
      )}
    </div>
  );
