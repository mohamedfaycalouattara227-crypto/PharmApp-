/**
 * ScreenUtilisateurs — Gestion des comptes utilisateurs (§4.1 CDC, étape 9).
 *
 * Extrait de _app.utilisateurs.tsx lors du découpage modulaire (v2).
 * La route conserve uniquement la définition TanStack Router.
 *
 * CRUD complet, suspension/réactivation, déverrouillage anti-brute-force.
 * Accès réservé au titulaire et à l'administrateur.
 */

import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { useState } from "react";
import { UserCog, Plus, Pencil, Lock, Unlock, UserX, UserCheck, X } from "lucide-react";
...
type UtilisateurForm = {
  ...
  role: RoleUtilisateur; numero_ordre: string; est_actif: boolean; mot_de_passe: string;
};
...
function UtilisateurDialog({ initial, onSave, onClose }: {
  initial?: Utilisateur | null;
  onSave: (d: Partial<UtilisateurForm>) => Promise<void>;
  onClose: () => void;
}) {
  const [form, setForm]     = useState<UtilisateurForm>(initial ? {
    ...
    numero_ordre: initial.numero_ordre ?? "", est_actif: initial.est_actif, mot_de_passe: "",
  } : FORM_VIDE);
  const [erreur, setErreur] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);

  const champ = (k: keyof UtilisateurForm) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => setForm((f) => ({ ...f, [k]: e.target.value }));
  const bool  = (k: keyof UtilisateurForm) => (e: React.ChangeEvent<HTMLInputElement>) => setForm((f) => ({ ...f, [k]: e.target.checked }));
...
export function ScreenUtilisateurs() {
  ...
  const creer        = useMutation({ mutationFn: (d: unknown) => api.utilisateurs.creer(d),                            onSuccess: () => qc.invalidateQueries({ queryKey: ["utilisateurs"] }) });
  const modifier     = useMutation({ mutationFn: ({ id, d }: { id: string; d: unknown }) => api.utilisateurs.modifier(id, d), onSuccess: () => qc.invalidateQueries({ queryKey: ["utilisateurs"] }) });
  const suspendre    = useMutation({ mutationFn: (id: string) => api.utilisateurs.suspendre(id),                       onSuccess: () => qc.invalidateQueries({ queryKey: ["utilisateurs"] }) });
  const reactiver    = useMutation({ mutationFn: (id: string) => api.utilisateurs.reactiver(id),                       onSuccess: () => qc.invalidateQueries({ queryKey: ["utilisateurs"] }) });
  const deverrouiller = useMutation({ mutationFn: (id: string) => api.utilisateurs.deverrouiller(id),                  onSuccess: () => qc.invalidateQueries({ queryKey: ["utilisateurs"] }) });
    ...
    <div className="flex h-screen flex-col overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-6 py-3 border-b border-border/40">
        <div className="relative max-w-md flex-1">
          <input className="input pl-4" placeholder="Rechercher nom, email…" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      </div>

      <div className="flex-1 overflow-auto px-6 py-4">
        {isLoading ? <div className="flex justify-center py-16 text-muted-foreground">Chargement…</div>
          : utilisateurs.length === 0 ? <div className="flex flex-col items-center gap-4 py-20 text-muted-foreground"><UserCog className="h-12 w-12" /><p>Aucun utilisateur trouvé.</p></div>
          : (
            <div className="space-y-2">
              {utilisateurs.map((u) => {
                const estMoi = u.id === moi;
                return (
                  <div key={u.id} className={`flex items-center gap-4 rounded-xl border ${u.est_actif ? "border-border/70 bg-surface/40" : "border-border/40 bg-surface/20 opacity-60"} px-5 py-4`}>
                    <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
                      <span className="text-sm font-semibold text-primary">{(u.prenom[0] ?? "") + (u.nom[0] ?? "")}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="font-medium">{u.prenom} {u.nom}</span>
                        {estMoi && <span className="text-xs text-muted-foreground">(vous)</span>}
                        {u.est_verrouille && <span className="flex items-center gap-1 text-xs text-destructive"><Lock className="h-3 w-3" /> Verrouillé</span>}
                      </div>
                      <div className="text-sm text-muted-foreground">{u.email}</div>
                      <div className="flex items-center gap-3 mt-1">
                        <span className="rounded-full px-2 py-0.5 text-xs bg-primary/10 text-primary">{LIBELLES_ROLE[u.role] ?? u.role}</span>
                        {u.derniere_connexion_reussie && <span className="text-xs text-muted-foreground">Dernière connexion : {fmtDate(u.derniere_connexion_reussie)}</span>}
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      {u.est_verrouille && !estMoi && <button onClick={() => deverrouiller.mutate(u.id)} className="btn btn-ghost flex items-center gap-1.5 text-xs py-1.5"><Unlock className="h-3.5 w-3.5" /> Déverrouiller</button>}
                      {!estMoi && (u.est_actif
                        ? <button onClick={() => { if (confirm(`Suspendre ${u.prenom} ${u.nom} ?`)) suspendre.mutate(u.id); }} className="btn btn-ghost flex items-center gap-1.5 text-xs py-1.5 text-warning"><UserX className="h-3.5 w-3.5" /> Suspendre</button>
                        : <button onClick={() => reactiver.mutate(u.id)} className="btn btn-ghost flex items-center gap-1.5 text-xs py-1.5 text-success"><UserCheck className="h-3.5 w-3.5" /> Réactiver</button>
                      )}
                      <button onClick={() => setDialog({ open: true, item: u })} className="btn btn-ghost flex items-center gap-1.5 text-xs py-1.5"><Pencil className="h-3.5 w-3.5" /> Modifier</button>
                    </div>
                  </div>
                );
              })}
